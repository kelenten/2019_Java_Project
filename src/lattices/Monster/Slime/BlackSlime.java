...
public class BlackSlime extends Monster {
    private int blood = 200;
    private int attack = 45;
    private int defence = 15;
    private int gold = 5;
    private int criticalAttackRate = 33;
    private int dodgeRate = 33;
    ...
    public void getAnimation(StackPane stackPane){
        Image p = new Image("file:pic/Monsters/黑史莱姆.png");
        ImageView v = new ImageView(p);
        stackPane.getChildren().add(v);
        TranslateTransition translateTransition = new TranslateTransition(Duration.millis(900) , v);
        ...
        loop:while (exist){
            ...
            if(this.blood > 0) {
                ...
                if(player.getBlood() <= 0) {
                    System.out.println("你被黑史莱姆击杀，游戏失败！");
                    System.exit(0);
                }
            }else {
                System.out.println("击败黑史莱姆!");
                player.setGold(player.getGold() + this.gold);
                player.setExp(player.getExp() + 400);
    ...
    public String virtualAttack(Player player){
        ...
        loop:while (true){
            ...
            if(vBlood > 0) {
                ...
                if(player.getBlood() <= 0) {
                    return "你可能会被黑史莱姆击杀!共受到" + total+"点伤害，其暴击了"+cAF+"次，"+"躲避了"+dF+"次";
                }
            }else {
                player.setBlood(player.getBlood());
                player.setGold(player.getGold() + this.gold);
                return "你可能击杀黑史莱姆。共受到" + total+ "点伤害，其暴击了"+cAF+"次，"+"躲避了"+dF+"次";
            }
        }
    }
    ...
    public ImageView getPic(){
        ImageView imageView;
        if(exist){
            imageView = new ImageView(new Image("file:pic/Monsters/黑史莱姆.png"));
            return imageView;
        } else{
