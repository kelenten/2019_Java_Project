...
public class SkeletonCaptain extends Monster {
    private int blood = 250;
    private int attack = 130;
    private int defence = 10;
    private int gold = 15;
    private int specialAttackRate = 33;
    ...
    public void getAnimation(StackPane stackPane){
        Image i1 = new Image("file:pic/Monsters/骷髅队长1.png");//形态一
        Image i2 = new Image("file:pic/Monsters/骷髅队长2.png");//形态二
    ...
    public void affectWith(Player player) {
        ...
        loop:while (exist){
            if (player.getAttack() > defence) {
                attackFrequency++;
                this.blood -= (player.getAttack() - defence);
            }else{
            ...
            if(this.blood > 0) {
                ...
                if(player.getBlood() <= 0) {
                    System.out.println("你被骷髅队长击杀，游戏失败！");
                    System.exit(0);
                }
            } else {
                System.out.println("击败骷髅队长！");
                player.setGold(player.getGold() + gold);
                player.setExp(player.getExp() + 600);
    ...
    public String virtualAttack(Player player){
        ...
        while (true){
            ...
            if(vBlood> 0) {
                ...
                if(player.getBlood() <= 0) {
                    return "你可能被骷髅队长击杀！共受到伤害"+total+"，其中特殊攻击"+sAF+"次";
                }
            } else {
                player.setGold(player.getGold() + gold);
                return "你可能击败骷髅队长，共受到伤害" +total + "，其中特殊攻击"+sAF+"次";
            }
        }
    }
    ...
    public ImageView getPic(){
        ImageView imageView;
        if(exist){
            imageView = new ImageView(new Image("file:pic/Monsters/骷髅队长1.png"));
        } else{
